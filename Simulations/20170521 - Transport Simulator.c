  ...
  for(i=0 ; i < regnum ; i++) {
    ...
    for(k=0 ; k<10 ; k++) {  //Scan through the first ten characters in 'skip'.
      if(skip[k] == ',') {  //There is a special-handling code.
        ...
        if(cplanes[reg][2] == 2. || cplanes[reg][2] == 7. || cplanes[reg][2] == 9.) fscanf(connexfp,"(%lf)",&cplanes[reg][3]); //If the cut-plane has special-handling code 2 then read in the associated value.
...
//Calculate cut-plane orientations and put them in 'cplanes'. Also find basepoints offsets for T connections and set appropriate internal special handling codes.
  ...
  for(i = 0 ; i < regnum ; i++) {
    ...
    if(cplanes[reg][2] == 9) cplanes[reg][1] = cplanes[reg][1] + cplanes[reg][3];
  }
